...
api_table_name returns [String ret]
    :   n=NAME {
            $ret = n.getText();
        }
    ;

api_column_name returns [String ret]
    :   n=NAME { $ret = n.getText(); }
    ;

api_param_defs returns [List<String> ret]
    :
        v = api_column_def {
            $ret = new ArrayList<String>();
            $ret.add(v.ret);
        } (AND v = api_column_def {$ret.add(v.ret);} )*
    ;

api_column_def returns [String ret]
    :   c=api_column_name
        {
            $ret = c.ret;
        }
...
get_api_statement
    : GET table_name=api_table_name BY params=api_param_defs
    {
        dynamicApiCallInfo.setTableName(table_name.ret);
        dynamicApiCallInfo.setApiName($text);
        dynamicApiCallInfo.setColumns(params.ret);
    }
    ;

WS : WHITESPACE+ { $channel = HIDDEN;};
NAME : (UNICODE_LETTER|UNICODE_DIGIT|LETTER|DIGIT|UNDERLINE) (UNICODE_LETTER|UNICODE_DIGIT|LETTER|DIGIT|UNDERLINE)*;
